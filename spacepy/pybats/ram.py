...
class LogFile(PbData):
    ...
    special methods for plotting common RAM-SCB log file values, such as
    D$_{ST}$. Supports RAM log files and SCB dst files.

    .. autosummary::

        ~LogFile.add_dst_quicklook
    ...
    def read(self):
        ...
        # Parse the header, detect RAM log or SCB Dst file
        firstline = raw.pop(0).strip()
        self.attrs['scb'] = True if firstline.startswith('SCB') else False
        self.attrs['descrip'] = firstline
        # Next line has variable names
        namevar = raw.pop(0).strip().split()
        nCols = len(namevar)
        loc={}
        ...
        for vname in ['year', 'mo', 'dy', 'hr', 'mn', 'sc', 'msc', 'time']:
            namevar.pop(namevar.index(vname))
        ...
        for i, line in enumerate(raw[:nPts]):
            vals = np.asarray(line.split(), dtype=float)
            # Set time:
    ...
    def add_dst_quicklook(self, target=None, loc=111, showObs=True,
                          showDPS=True, showBiot=True):
        '''
        Create a quick-look plot of Dst.

        Uses :module:`~spacepy.pybats.kyoto` to retrieve observed Dst for
        comparison to data.
        ...
        showDPS : bool
            Show Dessler-Parker-Scopke Dst?  Defaults to **True**
        showBiot : bool
        ...
        dpsvar = 'dstDPS' if self.attrs['scb'] else 'dstRam'
        dstlab = 'RAM-SCB Dst ({0})' if self.attrs['scb'] else 'RAM Dst ({0})'

        if dpsvar in self and showDPS:
            ax.plot(self['time'], self[dpsvar], label=dstlab.format('DPS'))
        if 'dstBiot' in self and showBiot:
            ax.plot(self['time'], self['dstBiot'], label=dstlab.format('Biot'))
